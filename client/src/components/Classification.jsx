    ...
    return (
        <div class="flex flex-col py-2 px-2">
            ...
            <div class="flex flex-col bg-[#252525] w-fit border-solid border-4 border-white rounded-md">
                {lapStatus.length > 0 ? lapStatus.map((el, index) => el.position != 0 ? <ClassificationRow key={index} position={el.position} teamId={participantsStatus[el.arrayIndex]?.teamId} driverId={participantsStatus[el.arrayIndex]?.driverId} lapTime={el.lastLapTimeInMS} tyreCompound={carStatus[el.arrayIndex]?.tyreCompound} /> : 
                null) : 
